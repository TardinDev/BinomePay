...
/**
 * Logo Binôme Pay animé.
 *
 * @param size      Largeur/hauteur en px (défaut: 300)
 * @param animated  Active les animations (défaut: true). À passer à false pour capture
 *                  ou quand l'animation doit être off (batterie, prefers-reduced-motion).
 */
export const Logo: React.FC<LogoProps> = ({ size = 300, animated = true }) => {
  ...
  useEffect(() => {
    ...
    const spawnRing = (ring: Animated.SharedValue<number>, delay: number) => {
      setTimeout(() => {
  ...
  const arc1Props = useAnimatedProps(() => ({
    transform: `rotate(${rotation1.value})`,
  }))

  const arc2Props = useAnimatedProps(() => ({
    transform: `rotate(${rotation2.value})`,
  }))
  ...
  const makeRingProps = (ring: Animated.SharedValue<number>) =>
    useAnimatedProps(() => ({
      r: interpolate(ring.value, [0, 1], [60, 105]),
      opacity: interpolate(ring.value, [0, 0.5, 1], [0.5, 0.3, 0]),
    }))

  const ring1Props = makeRingProps(ringPulse)
  const ring2Props = makeRingProps(ringPulse2)
  const ring3Props = makeRingProps(ringPulse3)
  ...
  return (
    <View style={[styles.container, { width: size, height: size }]}>
      <Svg width={size} height={size} viewBox="0 0 300 300">
        ...
        <Rect x="0" y="0" width="300" height="300" rx="66" fill="#000000" />
